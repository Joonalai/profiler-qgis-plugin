...
class MacroMouseDoubleClickEvent(MacroEvent):
    ...
    def perform_event_action_with_event(self) -> None:
        ...
        # Create and send mouse press/release events
        event = QMouseEvent(
            QEvent.MouseButtonDblClick,
            local_pos,
            global_pos,
            self.button,
            self.button,  # Send the same buttons as pressed buttons
            Qt.KeyboardModifiers(self.modifiers),
        )
        QApplication.postEvent(widget, event)
        QApplication.processEvents()
...
class MacroRecorder(QObject):
    ...
    def __init__(
        ...
        widgets_to_filter_events_out: Optional[list[QWidget]] = None,
    ) -> None:
        super().__init__(None)
        ...
        self._widgets_to_filter_events_out = (
            widgets_to_filter_events_out
            if widgets_to_filter_events_out is not None
            else []
        )
